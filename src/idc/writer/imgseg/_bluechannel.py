...
from idc.api import ImageSegmentationData, imgseg_to_bluechannel
from seppl.placeholders import placeholder_list, InputBasedPlaceholderSupporter


class BlueChannelImageSegmentationWriter(SplittableStreamWriter, AnnotationsOnlyWriter, InputBasedPlaceholderSupporter):
    ...
    def description(self) -> str:
        ...
        :rtype: str
        """
        return "Saves the annotations as blue channel PNG files. The associated JPG images can be placed in folder relative to the annotation."
    ...
    def write_stream(self, data):
        ...
        for item in make_list(data):
            ...
            if item.has_annotation():
                ann = imgseg_to_bluechannel(item.image_width, item.image_height, item.annotation, background=self.background)
                path = sub_dir
                os.makedirs(path, exist_ok=True)
                path = os.path.join(path, item.image_name)
