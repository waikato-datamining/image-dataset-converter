...
class VOCObjectDetectionReader(Reader, VariableSupporter):
    ...
    def _create_argparser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-i", "--input", type=str, help="Path to the XML file(s) to read; glob syntax is supported; " + variable_list(obj=self), required=False, nargs="*")
        parser.add_argument("-I", "--input_list", type=str, help="Path to the text file(s) listing the XML files to use; " + variable_list(obj=self), required=False, nargs="*")
    ...
    def read(self) -> Iterable:
        ...
        if self._inputs is None:
            self._inputs = locate_files(self.source, input_lists=self.source_list, fail_if_empty=True, default_glob="*.xml", resume_from=self.resume_from)
